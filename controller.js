    ...
    try {
        ...
        req.files.forEach((file) => {
            let image = {
                ...
                filePath: `https://react-eden-backend-production.up.railway.app/shop/${file.filename}`,
...
async function LoginUser(req, res, next) {
    try {
        ...
        if (await bcrypt.compare(password, user.password)) {
            const token = jwt.sign({ id: user._id }, Cipher, {
                expiresIn: "1h",
            })
...
async function GetUser(req, res, next) {
    try {
        ...
        getUser ? res.json({ user: user }) : res.json(null)
    } catch (error) {
        next(error)
    }
}
...
async function GetAllFavorites(req, res, next) {
    try {
        ...
        getUser ? Success(res, "User", getUser) : Failure(res, "No user found")
    } catch (error) {
        next(error)
    }
}
